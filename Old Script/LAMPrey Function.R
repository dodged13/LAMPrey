  LAMPrey = function(x,df,y,z,hk,f,c,p) {
    ...
    dat_list2 = replicate(length(unique(ct$gene)), data.frame(meanCT=(replicate(3,1)), sample = factor(c(rep(1,1),rep(2,1),rep(3,1))), deltactActin = rep(3,1),deltactELF1A = rep(3,1)), simplify = FALSE)
    ...
    if(missing(z)) {
      ...
      if (z == "graph"){
        return(ggplot(data = dat2, aes(x=Cycle, y= normalised)) + geom_line(aes(col = Well), size =1.25) + 
                 theme_minimal() + geom_hline(yintercept = p, linetype = 3, size = 1) + geom_vline(xintercept = c, linetype = 3, size = 1) + facet_wrap(~gene + sample, scales = "free") + theme(legend.position = "NONE") + labs(title = f)) }
